...
PIVXGUI::PIVXGUI(const NetworkStyle* networkStyle, QWidget* parent) :
    ...
    if (windowTitle.isEmpty()) {
        windowTitle = tr("Dash Diamond Core") + " - ";
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Dash Diamond Core client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Dash Diamond Core"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Dash Diamond Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
