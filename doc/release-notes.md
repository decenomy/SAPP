Sapphire Core version *1.4.0* is now available from:  <https://github.com/sappcoin-com/SAPP/releases>
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/Sapphire-Qt (on Mac) or sapphired/sapphire-qt (on Linux).
